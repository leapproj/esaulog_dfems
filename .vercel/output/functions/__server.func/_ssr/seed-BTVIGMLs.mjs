...
//#region node_modules/.nitro/vite/services/ssr/assets/seed-BTVIGMLs.js
...
async function runErpSeed(sql) {
	...
	await sql.query(`update license_packages set name = 'Starter Festival Website', price_php = 90000, billing = 'Full ownership (one-time license)', description = 'Festival website and TukodPH CMS. Calendar, about, partners, and a public programme — the digital front door.', features_json = $1 where id = 'pkg_site'`, [JSON.stringify([
		"Website + Content Management System (CMS)",
		"Public festival website",
		"Event calendar",
		"Draft content and auto publish to social media APIs",
	...
	await sql.query(`update license_packages set name = 'Organizer Command Center', price_php = 250000, billing = 'per season', description = 'Full DFEMS tenant: command center, ePASS, gate staff, participant portal, vendors, and sponsor activation.', features_json = $1 where id = 'pkg_command'`, [JSON.stringify([
		"Everything in Starter",
		"Organizer Command Center",
		"ePASS + printable credentials",
		"Gate-staff access keys",
	...
	await sql.query(`update license_packages set name = 'Smart Festival', price_php = 490000, billing = 'per season', description = 'Command plus turnout analytics, income ledgers, MSME Booster, and the AI Festival Organizer.', features_json = $1 where id = 'pkg_intel'`, [JSON.stringify([
		"Everything in Command",
		"Turnout & income analytics",
		"AI Festival Organizer",
		"Trade Fair Expo Cashless Transactions (Limited 2 Expo)",
...
async function runOperatorSeed(sql) {
	...
	for (const a of [
		...
		const existing = await sql.query(`select id from operator_accounts where id = $1 or lower(username) = $2 or lower(username) = $3`, [
			a.id,
			a.username.toLowerCase(),
			`tukodph_${a.username.toLowerCase()}`
		]);
		if (existing[0]) await sql.query(`update operator_accounts set username = $1, pass_hash = $2, kind = $3, display_name = $4, organization_name = $5, contact_email = $6 where id = $7`, [
			a.username,
			hashPass(a.pass),
			a.kind,
			a.display_name,
